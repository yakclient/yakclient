...
import dev.extframework.components.extloader.extension.feature.FeatureType
import dev.extframework.components.extloader.extension.feature.IllegalFeatureException
...
public class FeaturePartitionLoader : ExtensionPartitionLoader<FeaturePartitionMetadata> {
    ...
    override fun load(
        ...
        ExtensionPartitionContainer<FeaturePartitionNode, FeaturePartitionMetadata>(
            ...
            val extLoaderDefinedBuiltIn = "${dev.extframework.components.extloader.extension.partition.FeatureBuiltIn::class.java.name.withSlashes()}.class"
    ...
    private fun buildFeatureBuiltIn(
        ...
        val reader =
            ClassReader(ExtensionLoader::class.java.classLoader.getResourceAsStream(dev.extframework.components.extloader.extension.partition.FeatureBuiltIn::class.java.name.withSlashes() + ".class") ?: cantFindBuiltin())
        val node = ClassNode()
    ...
    private fun stubCall(
        ...
        val baseSource = Sources.of(dev.extframework.components.extloader.extension.partition.StubFeatureBuiltInCall::___stub___).get()
        incrementLocals(baseSource, argCount)

        baseSource
            .filterIsInstance<LdcInsnNode>()
            .filter { it.cst == dev.extframework.components.extloader.extension.partition.StubFeatureBuiltInCall.PARTITION_NAME_QUALIFIER }
            .forEach { it.cst = partition }

        baseSource
            .filterIsInstance<LdcInsnNode>()
            .filter { it.cst == dev.extframework.components.extloader.extension.partition.StubFeatureBuiltInCall.FEATURE_SIGNATURE_QUALIFIER }
            .forEach { it.cst = qualifiedSignature }

        baseSource
            .filterIsInstance<LdcInsnNode>()
            .filter { it.cst == Type.getType(dev.extframework.components.extloader.extension.partition.FeatureBuiltIn::class.java) }
